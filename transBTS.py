...
def create(image_size: typing.Tuple[int, int]):
    ...
    x = keras.layers.Conv2D(filters=8, kernel_size=3, strides=(2, 2), padding="same")(en1)  # (64, 64, 8)

    en2 = enblock(enblock(x, 8), 8)  # (64, 64, 8)

    x = keras.layers.Conv2D(filters=16, kernel_size=3, strides=(2, 2), padding="same")(en2)  # (32, 32, 16)

    en3 = enblock(enblock(x, 16), 16)  # (32, 32, 16)

    x = keras.layers.Conv2D(filters=32, kernel_size=3, strides=(2, 2), padding="same")(en3)  # (16, 16, 32)

    en4 = enblock(enblock(x, 32), 32)  # (16, 16, 32)

    lin_proj = keras.layers.Conv2D(filters=128, kernel_size=3, padding="same")(en4)  # (16, 16, 128)
    ...
    feat_map = keras.layers.Conv2D(filters=64, kernel_size=1, padding="same")(x)  # (16, 16, 64)
    feat_map = keras.layers.Conv2D(filters=32, kernel_size=1, padding="same")(feat_map)  # (16, 16, 32)

    x = keras.layers.Concatenate(axis=-1)([feat_map, en4])  # (16, 16, 64)
    x = enblock(enblock(x, 64), 64)  # (16, 16, 64)

    x = keras.layers.Conv2D(filters=64, kernel_size=3, padding="same")(x)  # (16, 16, 64)
    x = keras.layers.Conv2DTranspose(filters=64, kernel_size=3, padding="same", strides=2)(x)  # (32, 32, 64)
    x = keras.layers.Conv2D(filters=16, kernel_size=1, padding="same")(x)  # (32, 32, 16)

    x = keras.layers.Concatenate(axis=-1)([x, en3])  # (32, 32, 32)
    x = enblock(x, 32)  # (32, 32, 32)

    x = keras.layers.Conv2D(filters=32, kernel_size=3, padding="same")(x)  # (32, 32, 32)
    x = keras.layers.Conv2DTranspose(filters=32, kernel_size=3, padding="same", strides=2)(x)  # (64, 64, 32)
    x = keras.layers.Conv2D(filters=8, kernel_size=1, padding="same")(x)  # (64, 64, 8)

    x = keras.layers.Concatenate(axis=-1)([x, en2])  # (64, 64, 16)
    x = enblock(x, 16)  # (64, 64, 16)

    x = keras.layers.Conv2D(filters=16, kernel_size=3, padding="same")(x)  # (64, 64, 16)
    x = keras.layers.Conv2DTranspose(filters=16, kernel_size=3, padding="same", strides=2)(x)  # (128, 128, 16)
    x = keras.layers.Conv2D(filters=4, kernel_size=1, padding="same")(x)  # (128, 128, 4)
