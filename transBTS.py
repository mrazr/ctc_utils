...
def create_unet(image_size: typing.Tuple[int, int], data_format: str = 'channels_last'):
    global projection_dim
    projection_dim = (image_size[0] // 8) * (image_size[1] // 8)
    input = keras.layers.Input(shape=image_size)  # (w, h, 1) if `data_format` == 'channels_last'
    ...
    x = keras.layers.Conv2D(data_format=data_format, filters=32, kernel_size=3, padding='same')(en4)
    x = keras.layers.Conv2D(data_format=data_format, filters=32, kernel_size=3, padding='same')(x)

    x = keras.layers.Concatenate(axis=-1)([x, en4])  # (w/8, h/8, 64)
    x = enblock(enblock(x, 64), 64)  # (w/8, h/8, 64)
